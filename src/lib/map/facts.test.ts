...
describe('factsSql', () => {
	it('encodes the top categories as 1-based codes and escapes quotes', () => {
		const sql = factsSql(["Children's Game", 'Wargame']);
		expect(sql).toContain("WHEN 'Children''s Game' THEN 1");
		expect(sql).toContain("WHEN 'Wargame' THEN 2");
