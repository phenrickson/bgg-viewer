 ...
 */
import { query, queryColumns, nameOf } from '$lib/catalog/catalog.svelte';
import { fetchCoordinates, type CoordinateSet } from './coordinates';
import { alignFacts, factsSql, FACT_COLUMNS, TOP_CATEGORIES_SQL, type FactRowColumns, type GameFacts } from './facts';
...
export async function loadFacts(coords: CoordinateSet): Promise<GameFacts> {
	const top = await query<{ cat: string }>(TOP_CATEGORIES_SQL);
