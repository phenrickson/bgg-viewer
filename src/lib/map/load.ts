...
import { CATEGORIES, CATEGORY_PRIORITY } from './categories';
...
export async function loadFacts(coords: CoordinateSet): Promise<GameFacts> {
	...
	const cols = (await queryColumns(factsSql(labels, CATEGORY_PRIORITY), FACT_COLUMNS)) as unknown as FactRowColumns;
