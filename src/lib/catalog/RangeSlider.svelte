<script lang="ts">
   ...
   *
   * Built for complexity, which then went to band checkboxes (a set control, not a span
   * control), and year took steppers. Its first user is play time, which slides over tick
   * indices rather than minutes (`playtime.ts`). The rail's typed min/max pairs under "Exact
   * numbers" — rating, ratings count and geek rating — are genuine spans it would serve too.
   * `range.test.ts` covers the bounds logic.
   */
