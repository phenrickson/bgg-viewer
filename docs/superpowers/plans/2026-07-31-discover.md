...
## Revision — 2026-07-31, after the first visual review

Tasks 1–7 shipped a working page that fails on presentation. Seen next to the landing
page, the problem is not CSS but **framing**: the landing page is a centred column that
speaks in sentences, builds emphasis, and ends by pointing at a door. Discover was built
as a full-bleed filter toolbar above a list — bare uppercase labels, no opening, no
ending. It does not read as the same product, and it does not flow.

The arc must be **Home → Discover → Explore**, each page handing off to the next.

### Task 10: Reframe Discover as a continuation of the landing page

**Files:**
- Modify: `src/routes/(app)/discover/+page.svelte`
- Modify: `src/lib/discover/DialStrip.svelte`
- Modify: `src/lib/discover/GameRow.svelte`
- Modify: `src/lib/discover/dials.ts` (chip labels + the result limit)

**1. Measure and spine.** The page is full-bleed today. Wrap it in
`Container size="content"` (80rem — wider than the landing's 52rem `prose` because rows
carry more, far narrower than Explore's 112rem `wide`). This also dissolves the
"~900px gap between name and rating" problem reported in review: the row's `1fr` middle
column was dumping all surplus width into one gap, and the real fix is that the page was
the wrong shape. After capping, give the row's columns `minmax(floor, Nfr)` weights as
`GameList.svelte` already does, so any remaining surplus spreads instead of pooling.

**2. The dials ask questions.** Replace the filing-cabinet labels with the landing page's
voice — sentences, not field names:

- `CATEGORIES` → "What kind of game?"
- `PLAYERS` → "How many players?"
- `COMPLEXITY` → "How complex?"

Style the question like the landing page's `h1`/lede relationship, not like the `.try`
eyebrow: these are the page's content, not a form legend. Player chips read as answers
("Just me", "2 players", …), not bare digits — keeping the `aria-label` they already have.

**3. An opening.** The page currently starts cold on a label. Give it a short headline and
lede in the landing page's register, so arriving from Home feels continuous.

**4. An ending that points at Explore.** "See all N in Explore →" is currently a small
link floating at the right of the count line. Promote it to a real door at the BOTTOM of
the page, built like the landing page's `.door` (tinted panel, title + one line of
supporting copy), reading as the end of Discover's arc. Copy in the spirit of "Want to
explore more? Go to the catalog." Keep it carrying the current scope.

**5. A bounded, configurable result set.** Discover shows the top **25** by default, not
30,809. The count line becomes "Showing 25 of N matches" so nothing is hidden. Export the
limit as a named constant (`DISCOVER_LIMIT = 25`) from `dials.ts` so it is one edit to
change. Surfacing the full population here also dragged Explore's Universe concept onto a
page that exists to avoid it — the bounded set removes that.

**6. The complexity badge ramp.** Light→Heavy currently renders purple / blue / orange —
three unrelated hues read as categories, not as an ordered scale. `--chart-N` tokens are a
categorical palette and were the wrong source. Build the ramp from a SINGLE hue varying in
intensity (per `style-rules`: an ordered quantity needs an ordered encoding), keeping the
word as the primary carrier so the tint only reinforces it.

**Verification:** `pnpm exec vitest run`, `pnpm exec svelte-check`, then a visual pass in
BOTH themes at a wide viewport — the failure mode that produced this revision was invisible
to every diff-based review.
